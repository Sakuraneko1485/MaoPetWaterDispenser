...
void all_data_update(void)
{
    ...
    //mcu_dp_bool_update(DPID_SWITCH,Device_Switch); //当前开关，BOOL型数据上报;
    mcu_dp_enum_update(DPID_WORK_MODE,Work_Mode); //当前工作模式，枚举型数据上报;
    //mcu_dp_value_update(DPID_FILTER_DAYS,当前滤芯剩余天数); //当前滤芯剩余天数，VALUE型数据上报;
    //mcu_dp_value_update(DPID_PUMP_TIME,当前清洗剩余天数); //当前清洗剩余天数，VALUE型数据上报;
    //mcu_dp_bool_update(DPID_FILTER_RESET,当前滤芯重置); //当前滤芯重置，BOOL型数据上报;
    //mcu_dp_bool_update(DPID_PUMP_RESET,当前清洗重置); //当前清洗重置，BOOL型数据上报;
    //mcu_dp_value_update(DPID_FILTER_LIFE,当前滤芯更换提醒时间设置); //当前滤芯更换提醒时间设置，VALUE型数据上报;
    //mcu_dp_value_update(DPID_TIME_CLEAN,当前清洗提醒时间设置); //当前清洗提醒时间设置，VALUE型数据上报;
    mcu_dp_enum_update(DPID_WATER_LEVEL,water_level); //当前剩余水量档位，枚举型数据上报;
    mcu_dp_enum_update(DPID_WATER_CONTROL,Water_Control); //当前出水量调节，枚举型数据上报;
    mcu_dp_bool_update(DPID_UV,UV_Switch); //当前UV杀菌状态，BOOL型数据上报;
    //mcu_dp_bool_update(DPID_UV,uv); //当前UV杀菌状态，BOOL型数据上报;
    mcu_dp_bool_update(DPID_WARM,Warm_Switch); //当前恒温状态，BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_SET,Temp_Set); //当前温度设置，VALUE型数据上报;
...
static unsigned char dp_download_work_mode_handle(const unsigned char value[], unsigned short length)
{
    ...
    switch(work_mode) {
        ...
        Water_Control = 90;
        water_control = 1;
        EEPROM.update(E_SBSpeed, Water_Control); 
        Serial.println("工作模式设为普通模式");
        break;
        ...
        water_control = 0;
        EEPROM.update(E_SBSpeed, Water_Control); 
        Serial.println("工作模式设为夜间模式");
        break;
        
        case 2:
        Work_Mode = pir;
        Serial.println("工作模式设为感应模式");  
        break;
...
/*****************************************************************************
函数名称 : dp_download_pump_reset_handle
功能描述 : 针对DPID_PUMP_RESET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_filter_life_handle
功能描述 : 针对DPID_FILTER_LIFE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_time_clean_handle
功能描述 : 针对DPID_TIME_CLEAN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_water_control_handle
功能描述 : 针对DPID_WATER_CONTROL的处理函数
                                                         出水量档位调节
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
...
static unsigned char dp_download_water_control_handle(const unsigned char value[], unsigned short length)
{
    ...
    switch(water_control) {
        ...
        Water_Control = 70;
        EEPROM.update(E_SBSpeed, Water_Control); 
        Serial.println("出水量调为低");
        break;
        
        case 1:
        Water_Control = 90;
        EEPROM.update(E_SBSpeed, Water_Control); 
        Serial.println("出水量调为中");
        break;
        
        case 2:
        Water_Control = 100;
        EEPROM.update(E_SBSpeed, Water_Control); 
        Serial.println("出水量调为高");
        break;
...
static unsigned char dp_download_uv_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(uv == 0) {
        //开关关
        UV_Switch = 0;
        Serial.println("关闭UV杀菌功能");
    }
    else {
        //开关开
        UV_Switch = 1;
        Serial.println("开启UV杀菌功能");
    }
...
/*****************************************************************************
函数名称 : dp_download_warm_handle
功能描述 : 针对DPID_WARM的处理函数
                                                         恒温开关控制
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
...
static unsigned char dp_download_warm_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(warm == 0) {
        //开关关
        Warm_Switch = 0;
        Serial.println("关闭水温加热功能");
    }else {
        //开关开
        Warm_Switch = 1;
        Serial.println("开启水温加热功能");
    }
...
static unsigned char dp_download_temp_set_handle(const unsigned char value[], unsigned short length)
{
    ...
    EEPROM.update(E_TempSet, temp_set); 
    Serial.print("水温设置为");
